...
(function () {
  ...
  var state = {
    ...
    tshirtPurchaseReason: "Walk-in", // Order T-Shirt form's in-progress Reason field (Walk-in/Member/Complimentary)
    tshirtPurchaseSize: "",    // Order T-Shirt form's in-progress T-Shirt Size field (e.g. "Men's Large")
    tshirtPurchaseCount: "1",  // Order T-Shirt form's in-progress Count field — how many of that size this purchase covers
    ...
    tshirtPurchaseNote: "",    // Order T-Shirt form's in-progress Note field (free text)
    tshirtPurchaseEditId: null, // id of the purchase being edited (Edit button), or null when the form is for a new one
  ...
  function exportSummaryCsv() {
    ...
    section("Total Shirts Needed For Event (paid registrations + all sponsors + non-Walk-in Order T-Shirt purchases)");
    ...
    CONFIG.SIZES.forEach(function (sz) {
      ...
      row([sz.label, regMens + sponsorCounts[sz.key].mens + compCounts[sz.key].mens, regWomens + sponsorCounts[sz.key].womens + compCounts[sz.key].womens]);
    });

    section("Additional T-Shirt Purchases");
    row(["Purchases", state.tshirtPurchases.length]);
    row(["Shirts", tshirtPurchaseShirtTotal()]);
  ...
  function buildSummaryView() {
    ...
    container.appendChild(el("div", { class: "panel" }, [
      el("div", { class: "cards sponsor-cards" }, [
        ...
        el("div", { class: "sponsor-card" }, [
          ...
          el("div", { style: "font-size:11px; color:var(--muted); margin:-6px 0 8px", text: "Paid registrations + all sponsors + non-Walk-in Order T-Shirt purchases" }),
          combinedShirtMatrix()
        ])
      ])
    ]));
    ...
    container.appendChild(el("div", { class: "panel" }, [
      el("div", { class: "cards sponsor-cards" }, [
        el("div", { class: "sponsor-card" }, [
          el("div", { class: "sponsor-card-head", text: "Additional T-Shirt Purchases" }),
          el("div", { class: "sponsor-card-stats" }, [
            ...
            // Distinct from "Purchases" (transaction count) now that one
            // purchase can cover more than one shirt (the Order T-Shirt
            // form's Count field) — this is the actual shirt total.
            el("div", {}, [el("div", { class: "stat-v", text: String(tshirtPurchaseShirtTotal()) }), el("div", { class: "stat-k", text: "Shirts" })]),
  ...
  // `filterFn(purchase)`, if given, restricts which purchases count — e.g.
  // combinedShirtMatrix() only wants Complimentary ones (those genuinely
  // need to be ordered ahead of time; other Walk-In purchases are day-of
  // sales already fulfilled on the spot).
  function tshirtPurchaseShirtCounts(filterFn) {
    var C = CONFIG;
    var counts = {};
    ...
    state.tshirtPurchases.forEach(function (p) {
      ...
      if (!info || !counts[info.sizeKey]) return;
      // Pre-Count purchases have no `count` — treat as 1, same fallback the
      // Order T-Shirt table itself uses.
      var qty = Number(p.count) || 1;
      if (info.gender === "Men's") counts[info.sizeKey].mens += qty; else counts[info.sizeKey].womens += qty;
    });
    return counts;
  }
  // Total shirt quantity across every Walk-In T-Shirt purchase, summing each
  // purchase's Count (not just the number of purchase transactions) — same
  // pre-Count-field fallback (treat as 1) as tshirtPurchaseShirtCounts().
  function tshirtPurchaseShirtTotal() {
    return state.tshirtPurchases.reduce(function (sum, p) { return sum + (Number(p.count) || 1); }, 0);
  }
  ...
  // sponsor shirts plus every non-Walk-in-reason Order T-Shirt purchase
  // (Member, Complimentary, and any future reason), per size — deliberately
  // excludes only the "Walk-in" reason (a plain walk-up sale, fulfilled on
  // the spot from stock, not part of what needs to be ordered ahead of
  // time) — as opposed to the registration-only breakdown in shirtMatrix()
  // (which is unfiltered).
  function combinedShirtMatrix() {
    ...
    var rows = C.SIZES.map(function (sz) {
      ...
      var mens = regMens + sponsorCounts[sz.key].mens + compCounts[sz.key].mens;
      var womens = regWomens + sponsorCounts[sz.key].womens + compCounts[sz.key].womens;
      var cells = [
  ...
  // different places.
  // Includes paid registrants' shirts, sponsors' shirts (one row per shirt a
  // sponsor ordered — see LOGIC.sponsorShirtSizes, a sponsor can pick more
  // than one), AND Order T-Shirt (Walk-In) purchases — every place a shirt
  // gets handed out in this app, in one report. Sponsors aren't filtered by
  // any paid/unpaid status (there's no registration-style Status for a
  // sponsor), matching allSponsorShirtCounts()'s own precedent. `source`
  // (Registration/Sponsor/Walkin/Member/Complimentary) is what distinguishes
  // them in the report — see the "Source" column in TSHIRT_REPORT_ALL_COLS.
  var TSHIRT_PURCHASE_SOURCE_BY_REASON = { "Walk-in": "Walkin", "Member": "Member", "Complimentary": "Complimentary" };
  function tshirtReportRows() {
    var out = [];
    var sizeIndex = {};
    CONFIG.SIZES.forEach(function (sz, i) { sizeIndex[sz.key] = i; });
    var genderRank = { "Men's": 0, "Women's": 1 };
    function push(kind, source, row, gender, sizeKey, qty) {
      var label = gender + " " + sizeLabel(sizeKey);
    ...
    state.sponsors.forEach(function (sp) {
      ...
      LOGIC.sponsorShirtSizes(sp).forEach(function (size) {
        var info = CONFIG.SPONSOR_SIZE_INDEX[size];
        if (!info) return;
        push("sponsor", "Sponsor", sp, info.gender, info.sizeKey, 1);
      });
    });
    state.tshirtPurchases.forEach(function (p) {
      var info = p.size && CONFIG.SPONSOR_SIZE_INDEX[p.size];
      if (!info) return;
      var source = TSHIRT_PURCHASE_SOURCE_BY_REASON[p.reason] || "Walkin";
      push("purchase", source, p, info.gender, info.sizeKey, Number(p.count) || 1);
    });
    return out;
  }
  function tshirtReportCellText(pr, key) {
    ...
    if (key === "source") return pr.source;
    if (pr.kind === "sponsor") {
    ...
    if (pr.kind === "purchase") {
      if (key === "Last Name") return pr.row.name || "";
      if (key === "Status") return pr.source;
      return ""; // First Name, Reg # — no registrant-shaped equivalent
    }
  ...
  function tshirtReportSortValue(pr, key) {
    ...
    if (key === "source") return pr.source.toLowerCase();
    if (pr.kind === "sponsor") {
    ...
    if (pr.kind === "purchase") {
      if (key === "Last Name") return (pr.row.name || "").toLowerCase();
      if (key === "Status") return pr.source.toLowerCase();
      return "";
    }
  ...
  var TSHIRT_REPORT_SPEC = {
    ...
    defaultKeys: ["Last Name", "First Name", "source", "size", "qty"],
    defaultSortKey: "size",
  ...
  // Loads an existing purchase's fields into the form (same fields
  // addTshirtPurchase() reads) and switches it into edit mode — the Add
  // button becomes Save Changes, a Cancel button appears next to it (see
  // renderTshirtPurchasePage()), and saving updates that purchase in place
  // (same id, original purchasedAt preserved) instead of creating a new one.
  function editTshirtPurchase(id) {
    var p = state.tshirtPurchases.filter(function (x) { return x.id === id; })[0];
    if (!p) return;
    state.tshirtPurchaseEditId = id;
    state.tshirtPurchaseName = p.name || "";
    state.tshirtPurchaseCost = String(p.cost != null ? p.cost : 0);
    state.tshirtPurchaseReason = p.reason || "Walk-in";
    state.tshirtPurchaseSize = p.size || "";
    state.tshirtPurchaseCount = String(p.count || 1);
    state.tshirtPurchasePaymentType = p.paymentType || "Cash";
    state.tshirtPurchaseCheckNum = p.checkNum || "";
    state.tshirtPurchaseNote = p.note || "";
    state.tshirtPurchaseSyncError = null;
    renderTshirtPurchasePage();
  }
  function cancelEditTshirtPurchase() {
    state.tshirtPurchaseEditId = null;
    state.tshirtPurchaseName = "";
    state.tshirtPurchaseCost = String(state.appSettings.tshirtEventPurchaseCost || 0);
    state.tshirtPurchaseReason = "Walk-in";
    state.tshirtPurchaseSize = "";
    state.tshirtPurchaseCount = "1";
    state.tshirtPurchasePaymentType = "Cash";
    state.tshirtPurchaseCheckNum = "";
    state.tshirtPurchaseNote = "";
    state.tshirtPurchaseSyncError = null;
    renderTshirtPurchasePage();
  }
  function addTshirtPurchase() {
    ...
    var count = Number(state.tshirtPurchaseCount);
    ...
    if (!Number.isInteger(count) || count < 1) {
      state.tshirtPurchaseSyncError = "Count must be a whole number (1 or more).";
      renderTshirtPurchasePage();
      return;
    }
    if (paymentType === "Check" && !checkNum) {
    ...
    // Editing reuses the original id and purchasedAt (so it keeps its place
    // in the date-sorted table and doesn't look like a brand-new purchase);
    // adding generates both fresh.
    var editId = state.tshirtPurchaseEditId;
    var existing = editId ? state.tshirtPurchases.filter(function (p) { return p.id === editId; })[0] : null;
    upsertTshirtPurchase({
      id: existing ? existing.id : ("ts" + Date.now() + Math.random().toString(36).slice(2)),
      purchasedAt: existing ? existing.purchasedAt : new Date().toISOString(),
      name: name,
      cost: cost,
      ...
      paymentType: paymentType,
      checkNum: paymentType === "Check" ? checkNum : "",
      note: (state.tshirtPurchaseNote || "").trim()
    });
    cancelEditTshirtPurchase(); // also resets the form fields and re-renders
  }
  ...
  function renderTshirtPurchasePage() {
    ...
    ["Walk-in", "Member", "Complimentary"].forEach(function (r) {
      reasonSelect.appendChild(el("option", { value: r, text: r }));
    });
    reasonSelect.value = state.tshirtPurchaseReason || "Walk-in";
    reasonSelect.addEventListener("change", function () {
      state.tshirtPurchaseReason = reasonSelect.value;
      // Complimentary shirts aren't sold — default (not force; an officer can
      // still override) Cost to $0 and Payment Type to Gratis so the common
      // case needs no extra clicks. Full re-render (not just an in-place
      // field update) since both fields' displayed values change together.
      if (reasonSelect.value === "Complimentary") {
        state.tshirtPurchaseCost = "0";
        state.tshirtPurchasePaymentType = "Gratis";
      }
      renderTshirtPurchasePage();
    });
    body.appendChild(el("div", { class: "form-row" }, [
    ...
    // How many of that size this one purchase covers — e.g. a parent buying
    // shirts for the whole family in one transaction, instead of adding a
    // separate row per shirt.
    var countInput = el("input", { type: "number", min: "1", step: "1", value: state.tshirtPurchaseCount || "1", style: "width:100%" });
    countInput.addEventListener("input", function () { state.tshirtPurchaseCount = countInput.value; });
    body.appendChild(el("div", { class: "form-row" }, [
      el("span", { class: "form-label", text: "Count" }), countInput
    ]));
    ...
    var paymentTypeSelect = el("select", {});
    ["Cash", "Check", "Credit Card", "Gratis"].forEach(function (t) {
      paymentTypeSelect.appendChild(el("option", { value: t, text: t }));
    });
    ...
    var noteInput = el("input", { type: "text", placeholder: "Optional note", value: state.tshirtPurchaseNote || "" });
    noteInput.addEventListener("input", function () { state.tshirtPurchaseNote = noteInput.value; });
    body.appendChild(el("div", { class: "form-row" }, [
      el("span", { class: "form-label", text: "Note" }), noteInput
    ]));
    ...
    var editing = !!state.tshirtPurchaseEditId;
    var addBtn = el("button", { class: "btn primary" }, [editing ? "Save Changes" : "Add T-Shirt Order"]);
    addBtn.addEventListener("click", addTshirtPurchase);
    var actionRowKids = [addBtn];
    if (editing) {
      var cancelEditBtn = el("button", { class: "btn" }, ["Cancel"]);
      cancelEditBtn.addEventListener("click", cancelEditTshirtPurchase);
      actionRowKids.push(cancelEditBtn);
    }
    var actionRow = el("div", { class: "settings-actions" }, actionRowKids);
    ...
    if (!purchases.length) {
      ...
      var rows = purchases.map(function (p) {
        var editBtn = el("button", { class: "btn", style: "padding:2px 8px; font-size:12px" }, ["Edit"]);
        editBtn.addEventListener("click", function () { editTshirtPurchase(p.id); });
        ...
        return el("tr", {}, [
          ...
          el("td", { text: p.size || "—" }),
          // Same fallback reasoning as `reason` above — purchases recorded
          // before Count existed have no `count`, so those were always 1.
          el("td", { text: String(p.count || 1) }),
          el("td", { text: fmtMoney(p.cost) }),
          el("td", { text: p.paymentType || "—" }),
          el("td", { text: p.paymentType === "Check" ? (p.checkNum || "—") : "" }),
          el("td", { text: p.note || "" }),
          el("td", { style: "display:flex; gap:4px" }, [editBtn, delBtn])
        ]);
      });
      var total = purchases.reduce(function (sum, p) { return sum + (Number(p.cost) || 0); }, 0);
      var totalRow = el("tr", {}, [
        el("td", { colspan: "5", style: "text-align:right; font-weight:600", text: purchases.length + " purchase" + (purchases.length === 1 ? "" : "s") + " — Total" }),
        el("td", { style: "font-weight:600", text: fmtMoney(total) }),
        el("td", {}), el("td", {}), el("td", {}), el("td", {})
      ]);
      body.appendChild(el("table", { class: "matrix tshirt-purchases-table" }, [
        el("thead", {}, [el("tr", {}, [
