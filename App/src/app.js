...
(function () {
        ...
        .then(function (data) {
          if (data && data.ok && data.handledAt && (!requestedAt || new Date(data.handledAt) >= new Date(requestedAt))) {
            ...
            state.importRequestStatus = data.lastStatus === "failed"
              ? "Failed at " + when + (data.lastError ? ": " + data.lastError : "") + " — see the History tab or ask Claude to check the sync log."
              : "Succeeded at " + when + ".";
            stopImportRequestPolling();
            renderViews();
          }
  ...
  // after 7 days — see logs.php's CARSHOW_LOG_PURGE_DAYS. No local copy
  // exists anywhere anymore (see ETCCCarShowImportData's SKILL.md) — this is
  // the one and only place a run's log lives.
