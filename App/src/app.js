...
(function () {
  ...
  function requestImportNow() {
    ...
    fetch(SITE_CONFIG.importScheduleApiUrl, {
      ...
      .then(function (r) {
        state.importRequestStatus = (r.ok && r.data && r.data.ok)
          ? "Requested — the next scheduled check (within ~5 minutes) will run the import."
          : "Could not request an import — please try again.";
        renderViews();
      }).catch(function () {
  ...
  // /ETCCCarShowImportData Claude skill instead of a hardcoded URL that goes
  ...
  function buildImportScheduleSection() {
    ...
    if (state.importRequestStatus) importNowRow.appendChild(el("span", { class: "count" }, [state.importRequestStatus]));
      ...
      var table = el("table", { class: "grid" }, [
        ...
        el("tbody", {}, rows.map(function (r) {
          return el("tr", {}, [
            ...
            el("td", { style: "max-width:260px; overflow-wrap:break-word" }, [r.eventUrl || "—"])
          ]);
        }))
      ]);
      body = table;
