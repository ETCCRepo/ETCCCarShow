...
// of the /ETCCCarShowImportData Claude skill -- everything else that skill did
// (the check/mark_start/mark_run bridge, the upload, the log archive) was
// already plain HTTP and now lives in sync-registrations.js.
//
// Ported from the sibling Vette Fest app (Z:\Backup\Websites\VetteFest\App\
// deploy\clubexpress.js), where every ClubExpress quirk below was found and
// verified against the live site on 2026-09-12. Two fixes were added while
// porting, then ported back to Vette Fest the same day -- keep the copies in sync:
//   - normalizeClubExpressUrl() also repairs page_id=4091 -> 4055 (this app's
//     stored Event URL WAS still 4091 when this was ported; the first live run
//     only succeeded because of this repair);
//   - openContext() retries when the shared profile is busy (see its comment).
// (A third, in clubexpress-login.js's sessionIsLive(), was ported back too.)
//
// Auth model: this NEVER logs in. It drives a dedicated, persistent Chrome
// profile that an officer logged into ClubExpress once by hand (see
// clubexpress-login.js). If that session has lapsed the run fails with a clear
// message rather than touching a login form. That keeps ClubExpress admin
// credentials off this machine entirely and out of reach of any automation bug.
//
...
// an officer whether they got a durable session or a throwaway one.
//
...
//
// DELIBERATELY THE SAME FOLDER as the Vette Fest app's sync. Both events live
// on the same ClubExpress site under the same officer login, so one signed-in
// profile serves both -- an officer signs in once, not once per app. The cost
// is that the two scheduled tasks can collide on it; see openContext().
const PROFILE_DIR = path.join(
  process.env.LOCALAPPDATA || path.join(process.env.USERPROFILE || ".", "AppData", "Local"),
  "ETCC", "clubexpress-profile"
);

// The two exports. `label` is the visible text of the radio button in the
// Event Exports dialog; `prefix` is the filename convention
// upload-registrations.js's newestMatching() scans for.
const EXPORTS = [
...
// Repairs the two URL mistakes that are easy to make by copying out of the
// address bar, and that both misreport themselves as "not logged in":
//
// 1. HOST. ClubExpress issues its auth cookie (MEMBER_TOKEN) HOST-ONLY on
//    www.etccwebsite.com -- not the apex domain, not a wildcard. A URL missing
//    "www." is silently unauthenticated: the browser withholds the cookie,
//    ClubExpress bounces to action=login, and it looks exactly like an expired
//    session. (Cost Vette Fest a debugging cycle on 2026-09-12.)
//
// 2. PAGE. page_id=4091 is the PUBLIC "Event View" -- no Exports button, and it
//    even renders "Member Login" to a signed-in admin. page_id=4055 is the
//    "(Admin Panels)" page for the SAME item_id, which is what the export needs.
//    Verified live side by side for this app's event on 2026-09-12, after a
//    pasted 4091 URL broke every import. Only 4091 is rewritten -- any other
...
// browser the officer logged in with, so the stored session is valid.
//
// Retries because PROFILE_DIR is shared with the Vette Fest sync (see above)
// and Chrome allows one process per profile folder. The two tasks poll on
// independent 15-minute clocks and a full run takes a few seconds, so a
// genuine collision needs both imports due at nearly the same instant -- rare,
// but an unattended job shouldn't fail on it. Chrome's error for a locked
// profile isn't a stable string across versions, so any launch failure is
// retried; a real launch problem just costs ~45s before it reports.
async function openContext({ headless = true, attempts = 4, waitMs = 15000 } = {}) {
  ...
  throw new Error(
    ...
    "Vette Fest sync or an open clubexpress-login.js window): " + String(lastErr && lastErr.message).split(/\r?\n/)[0]
  );
}
...
// CarShow and Vette Fest event pages, verified live 2026-09-12) -- matching on
...
// Distinguishes the ways this page can be "wrong", because the fixes are
...
async function assertAdminPanelsPage(page, eventUrl) {
  ...
  if (await looksLoggedOut(page)) {
    // Before blaming the session, rule out the host trap: landing anywhere but
    // the www host looks identical to being logged out, and "go log in again"
    // would be flatly wrong advice.
    let host = "";
...
// and navigating to it DIRECTLY is far more robust than driving the modal. The
// ids are read from the live page, so this follows the event automatically.
...
// One export attempt: load the popup form fresh, pick the report type, replay
// the Export postback, save the CSV. Any step failing throws; the caller retries.
async function attemptExport(page, popupUrl, label, destPath, log) {
  ...
  // (both confirmed 2026-09-12 in the Vette Fest app):
...
// upload-registrations.js expects. Hard cap of 3 attempts per file: an
// unattended run with no upper bound can loop for many minutes on a problem a
// person would have spotted instantly.
