// The unattended import loop for the CarShow app -- a drop-in replacement for
// the `carshow-sync-registrations` Claude Code scheduled task, driven by
// Windows Task Scheduler instead. Requires no Claude subscription.
//
//   node deploy/sync-registrations.js [--force] [--headed]
//
// Ported from the sibling Vette Fest app's sync-registrations.js. The server
// side is UNCHANGED: this speaks exactly the same check/mark_start/mark_run
// contract to import-schedule.php that the Claude task did, uploads through
// the same registrations-upload.php (by reusing upload-registrations.js
// verbatim), and archives its log through the same logs.php. The Setup tab's
// Import Schedule panel and the History tab keep working with no modification.
// Even the Setup tab's "Automation" heartbeat keeps working, because it is
// written by the `check` call itself, not by whoever makes it.
//
...
//
// Environment:
//   CARSHOW_SITE_PASSWORD  (required) the live site's login password, set as a
...
const BASE_URL = "https://etccapps.com/apps/carshow";
// Must match upload-registrations.js's default, which reads the CSVs back out
// of this same folder.
const EXPORTS_DIR = process.env.CARSHOW_EXPORTS_DIR ||
...
// (Gitignored by the repo's *.log rule.)
...
async function storeLog(name, text) {
  ...
  // A failed archive is NOT a failed run -- note it and carry on.
...
function runUpload(eventUrl, file) {
  return new Promise((resolve) => {
    const child = spawn(process.execPath, [path.join(__dirname, "upload-registrations.js")], {
      ...
      env: {
        ...process.env,
        CARSHOW_SITE_PASSWORD: password,
        CARSHOW_YEAR: year,
        CARSHOW_EXPORTS_DIR: EXPORTS_DIR,
        CARSHOW_EVENT_URL: eventUrl || "",
        CARSHOW_LOG_FILE: file,
      },
    });
    let out = "";
...
(async () => {
  ...
  // tab's Event URL). Skipping the call outright would leave eventUrl empty.
  let reason = "manual";
  let eventUrl = "";
  ...
  // The env var overrides nothing -- it only fills a gap the Setup tab left.
  if (!eventUrl) eventUrl = process.env.CARSHOW_EVENT_URL || "";
  if (!eventUrl) {
    recordFailure(
      "No ClubExpress Event URL configured -- set it in the app's Setup tab (Import Schedule > Event URL), " +
      "or set CARSHOW_EVENT_URL for a one-off run."
    );
    process.exit(1);
  }
  ...
  try {
    ...
    // --- Step 5: upload into the live app. Passes the URL as stored in the
    // Setup tab, so the History tab's audit trail matches what an officer sees.
