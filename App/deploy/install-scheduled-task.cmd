@echo off
rem Creates (or re-creates) the Windows scheduled task that runs the Car Show
rem ClubExpress import every 15 minutes. Downloaded from the app's Setup tab >
rem Import Schedule. Double-click it on the machine that holds the CarShow
...
setlocal
set "INSTALLER=Z:\Backup\Websites\CarShow\App\deploy\install-scheduled-task.ps1"

if not exist "%INSTALLER%" (
  echo Could not find:
  echo   %INSTALLER%
  echo.
  echo Run this on the machine that has the CarShow repo at that path,
...
echo Creating the "carshow-sync-registrations" scheduled task...
echo.
